 ...
 */
export async function rerankWithCrossEncoder(query, chunks, topK) {
    if (chunks.length === 0)
        return [];
    if (chunks.length <= topK)
        return chunks;
    const scored = await Promise.all(chunks.map(async (text) => ({
        text,
...
async function scorePair(query, document) {
    try {
        const response = await fetch(`https://api-inference.huggingface.co/models/${CROSS_ENCODER_MODEL}`, {
            method: "POST",
            headers: {
...
function fallbackScore(query, document) {
