...
export async function subQueryNode(state) {
    const config = MODE_CONFIG[state.mode];
    if (!config.useSubQueries) {
        return { subQueries: [state.rewrittenQuery || state.query] };
    }
    const subs = await activeTracker.track("sub_query", () => runSubQueries(state.rewrittenQuery || state.query, config.maxSubQueries));
...
export async function gradeNode(state) {
...
export async function webSearchNode(state) {
    const result = await activeTracker.track("web_search", () => wikipediaTool.invoke({ query: state.query }));
    return {
        contextText: result,
        usedWebSearch: true,
...
export async function tokenBudgetNode(state) {
    const config = MODE_CONFIG[state.mode];
...
export async function generateNode(state) {
    const config = MODE_CONFIG[state.mode];
    const answer = await activeTracker.track("generation", () => runAnswer(state.query, state.contextText || "", config.maxOutputTokens));
