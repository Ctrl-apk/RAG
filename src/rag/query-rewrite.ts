...
export async function rewriteQuery(query: string): Promise<string> {
  const rewritten = await chat(
    "You are a query translation expert. Rewrite the user question into a clear, " +
      "search-optimized query for document retrieval. Return ONLY the rewritten query, nothing else.",
    query,
