 ...
 */
export async function decomposeQuery(query: string, maxSubQueries: number): Promise<string[]> {
  const raw = await chat(
    "Decompose the user question into focused sub-queries for document search. " +
      `Return at most ${maxSubQueries} sub-queries, one per line. No numbering, no bullets.`,
    query,
